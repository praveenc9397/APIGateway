...
import com.pkc.springbootmicroservice3apigateway.security.jwt.JwtAuthorizationFilter;
...
public class SecurityConfig extends WebSecurityConfigurerAdapter
{
    ...
    protected void configure(HttpSecurity http) throws Exception
    {
        ...
        http.authorizeRequests().antMatchers("/api/authentication/**").permitAll()//login and register pre-path
            .antMatchers(HttpMethod.GET, "/gateway/course").permitAll().antMatchers("/gateway/course/**").hasRole(
                Role.ADMIN.name()).anyRequest().authenticated();
